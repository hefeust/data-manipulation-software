...
import { uids_intersector } from './uids-intersector.js'
...
//        return await Promise.all(installed)
    ...
    let collect_uids_count = 0

    const collect_uids = async (filters) => {
        const intersector = uids_intersector(memo_roles_names)
        let isect_idx = 0

        for (const key  of memo_roles_names) {
            const role = await get_role(key)
            const parts = await role.select_parts(filters[role.role_name])
        ...
        return intersector.get_intersected()
    }

    const collect_uids0 = async (filters) => {
        ...
        console.log('duck-installer counter = ', counter, 
            'collect_uids_count = ', collect_uids_count
        )
