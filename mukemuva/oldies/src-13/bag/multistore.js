...
export const create_multistore = (roles_names, options = {}) => {
    ...
    const set = async (bag, with_data) => {
        ...
        for await (const records of collected_records) {
            // console.log({ records })
            const promises = []
            ...
            for(const record of records)  {
                ...
                promises.push(item.set_data({ with_data }))
...
//        collect_idx++
        ...
        for await (const records of collected_records) {
            ...
            console.log({ SELECT_RECORDS: records.length })

            for (const record of records) {
                const { role_uid, part_uid, item_uid } = record
                ...
                console.log('RECORD', { role_uid, part_uid, item_uid })
