...
//                kn_cache.set(kn, kn_block)
                
                for (const entry of record) {
                    if(entry.hasOwnProperty(kn)) {
                        const part_block = await pool.get_data(entry[kn])

                        parts_cache.set(kn, part_block)
...
//                console.log(isect.length)
            ...
            if(counter === 0) {
                ...
                for(const kn of kns) {
                    const part_block = parts_cache.get(kn)
                    ...
                    if(index === 0) {
                        ...
                        item_uid = await pool.set_data(item_blovk)
                    } 
            ...
            for (const kn of kns) {
                ...
                for (const entry of record) {
                    if (entry.hasOwnProperty(kn)) {
                        ...
                        parts_cache.set(kn, part_block)
                        bag[kn] = part_block.payload.part_value
                        lists.push(part_block.related)
    ...
    const select_records = (filters) => {
        ...
        const indices = kns.map((kn) => {
            ...
            if (kn_block.lookups.has(filter)) {
                filtered = [{
                     [kn]: kn_block.lookups.get(filter) 
                }]
            } else if (typeof filter === 'function') {
                filtered = pvs.filter((pv) => filter(pv))
                    .map((pv) => {
                        return { [pv]: kn_block.lookups.get(pv) }
                    })
            } else if (filter === '*') {
                filtered = pvs.map((pv) => {
                        return { [pv]: kn_block.lookups.get(pv) }
