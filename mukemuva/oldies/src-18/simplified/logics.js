...
export const make_logics = (context) => {
    ...
    const install_keynames = async (keynames) => {
        ...
        if (context.boot_uid === null) {
            ...
            base_cache.set(context.boot_uid, boot_block)
        }
    ...
    const install_items = async (bag, with_data) => {
        ...
        const promises = records.map(async (record) => {
            ...
            for (const entry of record) {
                const { keyname, part_uid } = entry
                const kn_uid = boot_block.lookups.get(keyname)
                const part_key = kn_uid + ':' + part_uid
...
//                console.log(entry)
//                console.log(Array.from(parts_cache.keys() ))
//                console.log(part_key,   parts_cache.has(part_key ))
            ...
            if (isect.length > 0) {
                for (const item_uid of isect) {
                    const item_block = items_cache.get(item_uid)
                ...
                const item_uid = await pool.set_data(item_block)

                let index = 0

                items_cache.set(item_uid, item_block)
